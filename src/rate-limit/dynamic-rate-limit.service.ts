...
export class DynamicRateLimit {
  ...
  static dynamicTtl() {
    ...
    const adjustedTtl = DynamicRateLimit.defaultTTL * Math.exp(
        alpha * (cpu - 50) + beta * (memory - 50)
    );

    return Math.max(Math.min(adjustedTtl, DynamicRateLimit.defaultTTL * 5), this.defaultLimit);
  }
